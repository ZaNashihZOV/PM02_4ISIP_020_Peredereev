...
namespace PM02_4ISIP_020_Peredereev
{
    public partial class MainWindow : Window
    {
        ...
        private int SolveTransportationProblem(int[] supply, int[] demand, int[,] costs)
        {
            ...
            while (true)
            {
                ...
                // Если не нашли больше клеток, прерываем цикл
                if (minI == -1 || minJ == -1)
                    break;

                // Определяем количество для перевозки
                int quantity = Math.Min(supply[minI], demand[minJ]);

                // Заполняем клетку опорного плана
            ...
            // Формируем строку с опорным планом
            ...
            // Выводим опорный план в окне сообщения
